...
class CmakeFileWriter():
  ...
  def __init__(self,  sourceFolder, topCmFolder):
...
class KernelCmakeFileWriter(CmakeFileWriter):
  ...
  def __init__(self, sourceFolder, topDir):
    super().__init__(sourceFolder,topDir)
...
class BspCmakeFileWriter(CmakeFileWriter):
  ...
  def parseBspSwitch(self, switch):
    ...
    if -1 != idx:
      logger.info("Switch: {0}".format(switch.getName()))
