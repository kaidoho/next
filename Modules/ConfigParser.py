...
class CfgParser():
    ...
    with open(self.makeFile, 'r') as f:
      ...
      while line:
        ...
        if -1 != idx:
          ...
          sb = LibraryTarget(libName, self.__sourceDir)
  ...
  def findTargetHeaderFiles(self, headerPath, destPath):
    ...
    logger.info("Headerfile: {0}".format(headerFile))
...
class CpukitParser(CfgParser):
  ...
  def parseMakefile(self):
    logger.info("Starting to parse Kernel Makefile: {0}".format(self.makeFile))
    ...
    for i in range(len(self.libraryObjs)):
      ...
      logger.info("Found lib: {0} contains {1} source files".format(self.libraryObjs[i].getName(),
                                                                         self.libraryObjs[i].getNumberOfSourceFiles()))

    mWriter = KernelCmakeFileWriter( os.path.dirname(os.path.abspath(self.makeFile)), self.getSrcDir())
...
class BspParser(CfgParser):
  ...
  def __init__(self, topDir, makefileDir):
    super().__init__()
    self.setSrcDir(topDir)
  ...
  def parseMakefile(self):
    logger.info("Starting to parse BSP Makefile: {0}".format(self.makeFile))
    self.findTargets("project_lib_LIBRARIES = ")
    ...
    mWriter = BspCmakeFileWriter(logger, sourceFolder, self.getSrcDir())
